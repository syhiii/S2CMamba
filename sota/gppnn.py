...
class Net(nn.Module):
    def __init__(self, num_channels, base_filter=64):
        ...
        self.lr_blocks = nn.ModuleList([LRBlock(num_channels, base_filter) for i in range(n_layer)])
        self.pan_blocks = nn.ModuleList([PANBlock(num_channels, 1, base_filter, 1) for i in range(n_layer)])
